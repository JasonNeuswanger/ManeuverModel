# -*- coding: utf-8 -*-
import numpy as np
from numba import jit, int64, float64, boolean, jitclass, optional
...
@jit(float64(float64, float64, float64, float64), nopython=True)
def s_t(u_i, u_f, t, tau): # st = distance traveled at time t after beginning at speed u_i with thrust u_f
...
@jit(float64(float64, float64, float64, float64), nopython=True)
def u_t(u_i, u_f, t, tau): # ut = velocity at time t after beginning with speed u_i and thrust u_f
    if u_i == u_f: # move at constant speed u_i = u_f
        return u_i
    else:
        Q = (u_f + u_i) / (u_f - u_i)
...
@jit(float64(float64, float64, float64, float64), nopython=True)
def t_s(u_i, u_f, s, tau): # ts = time taken to travel distance s after beginning at speed u_i with thrust u_f
    if u_i == u_f: # move at constant speed u_i = u_f
        return s / u_i
    else:
        Q = (u_f + u_i) / (u_f - u_i)
