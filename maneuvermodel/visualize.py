...
def summarize_solution(solution, display = True, title = None, export_path = None, should_print_dynamics = True, detailed = False, plot_dpi = 132):
    ...
    if export_path is not None:
        ...
        if display == False: plt.close(fig)
...
def draw_ground_coords_path_3D(dynamicstuple, solution, figure, actual_capture_point = None, multiplier = 10, colorbar = False, predicted_capture_point_color = (1,1,0), actual_capture_point_color = (0,1,0)):
    ...
    arrw = []
    prevsol = []
    def ar(x): return multiplier*np.array(x) # multiplier here for difference in units between maneuver model and data (I use 10 to convert modeled cm -> measured mm)
    ...
    if actual_capture_point is not None:
        ...
        mlab.plot3d(np.array((multiplier*predicted_capture_point[0], actual_capture_point[0])), np.array((multiplier*predicted_capture_point[1], actual_capture_point[1])), np.array((multiplier*predicted_capture_point[2], actual_capture_point[2])), figure = figure, color = (1,1,1), tube_radius=0.7, opacity=0.5)        
